"""
Code used to generate figures in the Data Exploration Section of the report.
"""
import pandas as pd
...
# Boxplot to look at low and high degeration tracks.
location = {}
    ...
    for event in dataset['EventName'].unique():
        ...
        for driver in eventDataset['Driver'].unique():
            ...
            for stint in driverDataset['Stint'].unique():
                # Take average difference per stint from laps where the driver doesn't enter or exit the pit lane.
                ...
                if len(stintDataset) > 1 and not stintDataset['LapTime'].isna().any():
                    ...
                    # Keep an collection of average time per stint in a list for each location.
...
location_names = list(location.keys())
degradation_values = list(location.values())

# Create the box plot showing average lap time difference per track.
...
# Correlation testing for Lap Time.
...
# Collecting a dataframe of all pit stops.
...
# Correlation testing for pit stop.
...
# Plot effect of classified position vs the distance to the driver ahead when the pit stop was made.
DNFS = ['R', 'D', 'E', 'W', 'F', 'N', 'U']
