...
class UnifiedModelGRU(nn.Module):
    def __init__(self, input_size, hidden_size, pit_choices_num):
        super(UnifiedModelGRU, self).__init__()
        self.hidden_size = hidden_size
        self.autoregress = False
        
        self.rnn_initial = nn.GRU(input_size, hidden_size, batch_first=True)
        self.rnn_autogressive = nn.GRU(input_size + 2, hidden_size, batch_first=True)
    ...
    def forward(self, x):
        ...
        for timestep in range(seq_len):
            ...
            h_t, h_s_initial = self.rnn_initial(x_t, h_s_initial)  # LSTM processes the input
            ...
            if timestep < seq_len - 1:
                ...
                autoregress_out, h_s_auto = self.rnn_autogressive(autoregress_input, h_s_auto)
...
def train():
    model = UnifiedModelGRU(INPUT_SIZE, HIDDEN_SIZE, PIT_CHOICES_NUM)
    model.to(device)
