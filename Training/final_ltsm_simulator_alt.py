...
TELEMETRY_BRAKE_FN = nn.BCEWithLogitsLoss()
...
class AutoF1LSTM(nn.Module):
    def __init__(self, input_size, hidden_size):
        ...
        self.brake_prediction = nn.Sequential(
            nn.Dropout(p=DROPOUT),
            nn.Linear(hidden_size, 64),
            nn.ReLU(),
            nn.Dropout(p=DROPOUT),
    ...
    def forward(self, lap, h_s, c_s):
        ...
        brake = self.brake_prediction(h_t)
        coordinates = self.coordinate_prediction(h_t)

        return h_s, c_s, compound_decision, lap_time, speedi1, speedi2, speedfl, speedst, speedtelemetry, rpm, gear, throttle, brake, coordinates
...
def testing_loop(model, laps):
    ...
    for lap in range(laps_in_race - 1):
        h_s, c_s, compound_decisions, lap_times, speedi1, speedi2, speedfl, speedst, speedtelemetry, rpm, gear, throttle, brake, coordinates = model(laps[:,lap].unsqueeze(0), h_s, c_s)
    ...
    return model_compound_decisions, laps[:,1:,-4].squeeze(0).long().to(device), lap_times_simulated, laps[:,1:,0].squeeze(0).to(device), \
        ...
        brake_simulated, laps[:,1:,9].squeeze(0).long().to(device), coordinates_simulated, laps[:,1:,10:13].squeeze(0).to(device)
...
def stats(testing_dataloader, model):
    ...
    real_brake_telemetry_labels = []
    simulated_brake_telemetry_labels = []
    ...
    for race_data in testing_dataloader:
        ...
        real_brake_telemetry_labels.append(real_brake_telemetry.cpu().numpy())
        simulated_brake_telemetry_labels.append(torch.cat(predicted_brake_telemetry).detach().cpu().numpy().flatten())
    ...
    real_brake_telemetry_labels = np.concatenate(real_brake_telemetry_labels)
    simulated_brake_telemetry_labels = np.concatenate(simulated_brake_telemetry_labels).flatten()
    # Apply binary classifer
    simulated_brake_telemetry_labels = [1.0 if label >= 0.5 else 0 for label in simulated_brake_telemetry_labels]
    ...
    print("==========BRAKE=============")
    brake_precision = labeling_stats(real_brake_telemetry_labels, simulated_brake_telemetry_labels)
    print("========COORDINATES=========")
...
def training_loop(model, laps):
    ...
    brake_telemetry_simulations = []
    coordinates_telemetry_simulations = []

    laps_in_race = laps.shape[1]

    for lap in range(laps_in_race - 1):
        h_s, c_s, compound_decisions, lap_times, speedi1, speedi2, speedfl, speedst, speed_telemetry, rpm_telemetry, gear_telemetry, throttle_telemetry, brake_telemetry, coord_telemetry = model(laps[:,lap].unsqueeze(0), h_s, c_s)
        ...
        brake_telemetry_simulations.append(brake_telemetry)
    ...
    rpm_telemetry_loss = TELEMETRY_RPM_FN(torch.cat(speedtelemetry_simulations).view(-1).to(device), laps[:, 1:, 6].squeeze(0).to(device))
    ...
    brake_telemetry_loss = TELEMETRY_BRAKE_FN(torch.cat(brake_telemetry_simulations).view(-1).to(device), laps[:, 1:, 9].squeeze(0).to(device))
    coordinate_telemetry_loss = TELEMETRY_COORD_FN(torch.cat(coordinates_telemetry_simulations).to(device), laps[:, 1:, 10:13].squeeze(0).to(device))
    return lap_time_loss * LAP_TIME_SCALE + compound_decision_loss * COMPOUND_SCALE + speedi1_loss * SPEED_SCALE + speedi2_loss * SPEED_SCALE + speedfl_loss * SPEED_SCALE + speedst_loss * SPEED_SCALE + speedtelemetry_loss * SPEED_SCALE + rpm_telemetry_loss * TELEMETRY_RPM_SCALE + gear_telemetry_loss * TELEMETRY_GEAR_SCALE + throttle_telemetry_loss * TELEMETRY_THROTTLE_SCALE +  brake_telemetry_loss * TELEMETRY_BRAKE_SCALE + coordinate_telemetry_loss * TELEMETRY_COORD_SCALE

def plot_graph(experiment_id, losses, values):
    plt.figure(figsize=(8,5))
    LABELS = ["Compound", "LapTime", "SpeedI1", "SpeedI2", "SpeedFL", "SpeedST", "Speed", \
        "RPM", "nGear", "Throttle", "Brake", "Telemetry Coordinates"]
    COLOURS = [
        'blue', 'red', 'green', 'orange', 'purple', 
        'brown', 'pink', 'cyan', 'magenta', 'yellow'
    ]

    # Transpose so each sequence is independent.
    values = values.T
...
def train(experiment_id):
    ...
    for epoch in range(EPOCHS):
        for _, race_data in enumerate(training_dataloader):
            ...
            if iter_counter % BATCH_SIZE == 0 or (epoch == EPOCHS and iter_counter - 1 == len(training_dataset)):
                total_loss.backward()
